...
def compute_trajectory_visual_weighting(
    ...
    """
    Compute inter-trajectory reweighting factors (Eq. 9-11).
    
    For each trajectory, compute visual accumulation ϕ_i = sum(w_{i,t}),
    then within each rollout group apply mean-centered adjustment:
    α(ϕ_i) = 1 + (ϕ_norm - ϕ_avg)
    
    Args:
        visual_weights: (batch_size, response_length) - token-level visual focus scores
        response_mask: (batch_size, response_length) - mask for valid tokens
        index: (batch_size,) - prompt ID for each response
        rollout_n: number of responses per prompt (rollout group size)
        normalization: normalization method ('minmax')
        temperature: temperature for softmax normalization
        clamp_min: minimum clamp value for reweighting factor
        clamp_max: maximum clamp value for reweighting factor
    
    Returns:
        reweighting_factors: (batch_size,) - per-trajectory reweighting factors
    """
    batch_size, response_length = visual_weights.shape
    reweighting_factors = torch.ones(batch_size, device=visual_weights.device, dtype=torch.float32)
    
    # Step 1: Compute visual accumulation ϕ_i for each trajectory
    traj_scores = []
    ...
    for batch_idx in range(batch_size):
        ...
        valid_weights = visual_weights[batch_idx, valid_mask]
        traj_scores.append(valid_weights.sum().item())
    
    traj_scores_tensor = torch.tensor(traj_scores, device=visual_weights.device, dtype=torch.float32)
    
    # Step 2-3: Apply weighting within rollout groups
    if rollout_n > 1 and batch_size >= rollout_n:
        # Group trajectories: assume consecutive n trajectories form a rollout group
        # Note: This assumes the batch is organized such that rollout groups are consecutive
        # If _balance_batch has shuffled the data, this may not work correctly
        # In that case, we need to use a different grouping mechanism (e.g., based on prompt_id)
        
        num_groups = batch_size // rollout_n
        for group_idx in range(num_groups):
            start_idx = group_idx * rollout_n
            end_idx = start_idx + rollout_n
            
            # Get trajectory scores for this group
            group_scores = traj_scores_tensor[start_idx:end_idx]
            
            if len(group_scores) == 0:
                continue
            
            # Step 2: Normalize trajectory scores within the group using minmax
            if normalization == "softmax":
                # Softmax normalization
                group_scores_normalized = F.softmax(group_scores / temperature, dim=0)
            else:  # minmax
                # MinMax normalization: map to [0, 1] within the group
                min_score = group_scores.min()
                max_score = group_scores.max()
                score_range = max_score - min_score + 1e-8
                group_scores_normalized = (group_scores - min_score) / score_range
            
            # Step 3: Apply mean-centered adjustment: α(S(τ_i)) = 1 + (S_norm - S_avg)
            # This ensures the average weight is 1 within the group
            s_avg = group_scores_normalized.mean()
            group_weighting_factors = 1.0 + (group_scores_normalized - s_avg)
            
            # Clamp to prevent extreme values
            group_weighting_factors = torch.clamp(
                group_weighting_factors,
                min=clamp_min,
                max=clamp_max
            )
            
            # Assign weighting factors to this group
            reweighting_factors[start_idx:end_idx] = group_weighting_factors
    else:
        if normalization == "softmax":
            traj_scores_normalized = F.softmax(traj_scores_tensor / temperature, dim=0)
        else:  # minmax
            min_score = traj_scores_tensor.min()
            max_score = traj_scores_tensor.max()
            score_range = max_score - min_score + 1e-8
            traj_scores_normalized = (traj_scores_tensor - min_score) / score_range
        
        s_avg = traj_scores_normalized.mean()
        reweighting_factors = 1.0 + (traj_scores_normalized - s_avg)
        reweighting_factors = torch.clamp(reweighting_factors, min=clamp_min, max=clamp_max)
...
def compute_policy_loss(
    ...
    **kwargs,
    ...
    if loss_token_weights is not None:
        # Soft masking: use weights instead of hard mask
        # All tokens participate, but weighted by visual attention
        detached_loss_token_weights = loss_token_weights.detach()
        final_pg_loss = final_pg_loss * detached_loss_token_weights
        # Normalize by weighted sum instead of simple average
        # This ensures the loss scale is consistent
        weighted_mask = response_mask * detached_loss_token_weights
        final_pg_loss = average_loss(final_pg_loss, weighted_mask, mode=loss_avg_mode)
    elif loss_token_mask is not None:
        detached_loss_token_mask = loss_token_mask.detach()
        final_pg_loss = final_pg_loss * detached_loss_token_mask
        # Fix: Use selected tokens for normalization, not all tokens
        # This prevents reward hacking where model learns to shorten responses
        final_pg_loss = average_loss(final_pg_loss, loss_token_mask, mode=loss_avg_mode)
    else:
        final_pg_loss = average_loss(final_pg_loss, response_mask, mode=loss_avg_mode)
    
    # Metrics should still use response_mask for consistency
