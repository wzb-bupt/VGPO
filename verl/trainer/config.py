...
class AlgorithmConfig:
    ...
    visual_attention_threshold: float = 0.4
    """visual attention threshold κ for determining if a token is a 'visual token'"""
    gated_visual_compensation_start_ratio: float = 0.5
...
class PPOConfig:
    ...
    def post_init(self):
        ...
        # Intra-trajectory reweighting
        self.worker.actor.use_intra_trajectory_reweighting = self.algorithm.use_intra_trajectory_reweighting

        # Visual attention compensation
        self.worker.actor.use_visual_compensation = self.algorithm.use_visual_compensation
        self.worker.actor.visual_compensation_type = self.algorithm.visual_compensation_type
        self.worker.actor.visual_compensation_start_ratio = self.algorithm.visual_compensation_start_ratio
        self.worker.actor.visual_compensation_strength = self.algorithm.visual_compensation_strength
        self.worker.actor.visual_compensation_exponent = self.algorithm.visual_compensation_exponent
        self.worker.actor.visual_compensation_step_ratio = self.algorithm.visual_compensation_step_ratio

        # Gated visual compensation
        self.worker.actor.use_gated_visual_compensation = self.algorithm.use_gated_visual_compensation
        self.worker.actor.visual_attention_threshold = self.algorithm.visual_attention_threshold
        self.worker.actor.gated_visual_compensation_start_ratio = self.algorithm.gated_visual_compensation_start_ratio

        # Inter-trajectory reweighting
        self.worker.actor.use_inter_trajectory_reweighting = self.algorithm.use_inter_trajectory_reweighting
        self.worker.actor.inter_reweighting_gamma_late = self.algorithm.inter_reweighting_gamma_late
