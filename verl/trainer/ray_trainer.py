...
class ResourcePoolManager:
    ...
    def _check_resource_available(self):
        ...
        gpus_available = ray.cluster_resources().get("GPU", 0)
...
class RayPPOTrainer:
    ...
    def fit(self):
        ...
        # ── VGPO Configuration Summary ──
        algo = self.config.algorithm
        _vgpo_modules = []
        if getattr(algo, 'use_visual_compensation', False):
            _vc_str = f"  Visual Attention Compensation (Eq.5): ON  [β={getattr(algo, 'visual_compensation_strength', 0.3)}, schedule={getattr(algo, 'visual_compensation_type', 'linear')}]"
            _vgpo_modules.append(_vc_str)
            if getattr(algo, 'use_gated_visual_compensation', False):
                _gvc_str = f"  Gated Visual Compensation   (Eq.6): ON  [γ={getattr(algo, 'gated_visual_compensation_start_ratio', 0.5)}, κ={getattr(algo, 'visual_attention_threshold', 0.2)}]"
                _vgpo_modules.append(_gvc_str)
        if getattr(algo, 'use_intra_trajectory_reweighting', False):
            _vgpo_modules.append("  Intra-Trajectory Reweighting (Eq.7-8): ON")
        if getattr(algo, 'use_inter_trajectory_reweighting', False):
            _vgpo_modules.append("  Inter-Trajectory Reweighting (Eq.9-11): ON")
        if _vgpo_modules:
            print("\n" + "=" * 60)
            print("  VGPO (Visually-Guided Policy Optimization) ENABLED")
            print("=" * 60)
            for m in _vgpo_modules:
                print(m)
            print("=" * 60 + "\n")
        else:
            print("\n[VGPO] No VGPO modules enabled. Running standard PPO.\n")
        ...
        while self.global_step < self.training_steps:
            ...
            with timer("step", timing_raw):
                ...
                batch.meta_info["rollout_n"] = self.config.worker.rollout.n
